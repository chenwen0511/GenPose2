...
依赖外部 SAM3 服务（默认 `http://127.0.0.1:18003/infer`）及 VLM（`vlm.sam3_prompt` / `vlm.reason` / `vlm.missing_prompt`）；M3 Key 用环境变量 `ANTHROPIC_API_KEY` 或 `configs/secrets.local.json`。放置产物含 `place_destination.json`、`vis_place_dest.png`、`scene_place_dest.glb`。UI 默认开启 **Depth→RGB 对齐**。HTTP API 仍见上文第 9 节与 `doc/接口文档.md`。
