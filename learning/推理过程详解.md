...
1. **加载三个权重**：Score（扩散采样）、Energy（能量排序）、Scale（尺度/轴向细化）。料盘密排、只见边缘时的**从零重训练**见 [`料盘边缘场景训练.md`](./料盘边缘场景训练.md)。
...
`start.sh` 会按 `configs/conf.json` 列出 SAM3 / 双 VLM / 三网权重，并提醒**依赖服务需事先启动**（脚本不代启）。

| 页签 | 说明 |
|------|------|
| SAM3 分割 | 外部 SAM3 HTTP 文本分割 + 灰色深度点云 GLB（不上 RGB 色） |
...
依赖外部 SAM3 服务（默认 `http://127.0.0.1:18003/infer`）及 VLM（`vlm.sam3_prompt` / `vlm.reason` / `vlm.missing_prompt`）；M3 Key 用环境变量 `ANTHROPIC_API_KEY` 或 `configs/secrets.local.json`。放置产物含 `place_destination.json`、`vis_place_dest.png`、`scene_place_dest.glb`。UI 默认开启 **Depth→RGB 对齐**（自动估计偏移，失败回退手动 dx/dy）；3D 点云为灰色深度反投影，不上 RGB 色。HTTP API 仍见上文第 9 节与 `doc/接口文档.md`。
