#!/bin/bash
# 使用 datasets/train_set_blender_v2 从零训练 EnergyNet
# 默认 nohup 后台运行，终端日志写入 logs/；设 FOREGROUND=1 可前台跑。
#
# 用法:
#   bash scripts/train/train_energy_tray_0810.sh
#   tail -f logs/train_energy_tray_0810_*.log
#   FOREGROUND=1 bash scripts/train/train_energy_tray_0810.sh
set -euo pipefail
...
RUN_LOG="${RUN_LOG:-$ROOT/logs/train_energy_tray_0810_${TS}.log}"
PID_FILE="$ROOT/.pids/train_energy_tray_0810.pid"
...
echo "[train] EnergyNet  data=$DATA_PATH  ckpt_log=$LOG_DIR  bs=$BATCH_SIZE  epochs=$N_EPOCHS"
